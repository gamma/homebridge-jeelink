...
LaCrosseDTH.prototype = {
    parseValue: function( data ) {
        ...
        this.log.info( 'Sensor ID    : ' + data.id );
        this.log.debug('Type         : ' + data.type );
        this.log.debug('NewBattery   : ' + data.newBat );
        this.log.info( 'Temperatur   : ' + data.temperature );
        this.log.info( 'Humidty      : ' + data.humidity );
    ...
    configure: function( device ) {
        LaCrosseDTHSensor.prototype.configure.apply( device, [ this.platform ] );
        device.context.lastUpdate = Date.now();
...
LaCrosseDTHSensor.prototype.configure = function( platform ) {

    JeeLinkAccessory.prototype.configure.apply(this, Array.from(arguments));

    // Add Temperature
    (this.getService(Service.TemperatureSensor) || this.addService(Service.TemperatureSensor, this.displayName))
    ...
    // Add Humidity
    (this.getService(Service.HumiditySensor) || this.addService(Service.HumiditySensor, this.displayName))
    .getCharacteristic(Characteristic.CurrentRelativeHumidity)
    .on('get', this.getCurrentDataValues.bind(this))
    ;

    if ( !this.update ) { this.update = LaCrosseDTHSensor.prototype.update; }
...
LaCrosseDTHSensor.prototype.update = function() {
    ...
    this.getCurrentDataValues(function(foo, data) {
        // TemperatureSensor
        this.log.info( "update current Temperature:", _self.context.deviceID, data.temperature  );
        this
        ...
        // Humidity Sensor
        this.log.info( "update current Humidity:", _self.context.deviceID, data.humidity  );
        this
