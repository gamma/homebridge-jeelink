...
var debug       = require('debug')('JeeLink');
...
//    setInterval(this.update.bind(this), 5000);
    ...
    var temperature = this.addService(Service.TemperatureSensor, this.name);
    temperature.getCharacteristic(Characteristic.CurrentTemperature)
        .setProps({minValue: -50})
        .on('get', this.getCurrentDataValues.bind(this))
    ;

    var humidity = this.addService(Service.HumiditySensor, this.name);
...
LaCrosseDTHSensor.prototype.update = function() {
    debug(`Updating ${this.context.type} ${this.context.deviceID}`);

    // TemperatureSensor
    this.getCurrentDataValues(function(foo, data) {
        debug( "update current Temperature: " + data.temperature  );
        this
        ...
        debug( "update current Humidity: " + data.humidity  );
        this
